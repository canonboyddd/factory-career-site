...
function unwrap(raw) { return raw?.item || raw?.Item || raw || {}; }
function safeNumber(value) { const n=Number(value); return Number.isFinite(n)?n:0; }
function redact(text,secrets=[]) { let out=String(text||''); for(const secret of secrets){if(secret)out=out.split(secret).join('[redacted]');} return out.slice(0,1000); }
function parseBody(raw) { try{return raw?JSON.parse(raw):{};}catch(_){return{};} }
function errorDetail(body,raw,secrets){return body?.error_description||body?.error||body?.errors?.[0]?.errorMessage||body?.errors?.[0]?.message||redact(raw,secrets)||'Rakuten API error';}
function normalizeItems(body) {
  ...
  return rawItems.map(unwrap).map(item => {
    ...
    return {
      name:String(item.itemName||'').slice(0,180),price:safeNumber(item.itemPrice),
      url:affiliateUrl || itemUrl,affiliate:Boolean(affiliateUrl),image:imageOf(item),
      shop:String(item.shopName||'').slice(0,100),reviewAverage:safeNumber(item.reviewAverage),reviewCount:safeNumber(item.reviewCount),
      caption:String(item.itemCaption||'').replace(/<[^>]*>/g,'').slice(0,220),postageFlag:safeNumber(item.postageFlag)
    };
...
export async function onRequestGet({request,env}) {
  ...
  const url=new URL(request.url); const diag=url.searchParams.get('diag')==='1';
  const missing=[]; if(!appId)missing.push('RAKUTEN_APP_ID'); if(!accessKey)missing.push('RAKUTEN_ACCESS_KEY');
  ...
  async function callRakuten(useAffiliate) {
    ...
    const controller=new AbortController(); const timer=setTimeout(()=>controller.abort('upstream-timeout'),10000);
    try{
      const res=await fetch(endpoint,{headers:{'Origin':SITE_ORIGIN,'Referer':`${SITE_ORIGIN}/`,'Accept':'application/json','User-Agent':`factory-career-site/1.0 (+${SITE_ORIGIN}/)`},signal:controller.signal,cf:{cacheEverything:false}});
      const raw=await res.text(); return {res,raw,body:parseBody(raw)};
