(() => {
  ...
  function render(items, category, affiliateActive) {
    ...
    grid.querySelectorAll('[data-rakuten-link]').forEach(link => {
  ...
  async function load(category = 'work', hits = 8) {
    ...
    if (!res.ok || !data.ok) {
      const message = String(data?.detail || data?.error || `HTTP ${res.status}`).slice(0, 220);
      setStatus(`楽天APIエラー: ${message}`);
      return;
    }

    const items = Array.isArray(data.items) ? data.items.filter(item => item?.name && item?.url) : [];
    if (!items.length) {
      setStatus('楽天APIには接続できましたが、表示できる商品が0件でした。');
      return;
    }
  ...
  buttons.forEach(button => {
    button.addEventListener('click', () => {
      ...
      load(button.dataset.rakutenCategory || 'work', 8);
    });
  });

  load(root.dataset.category || 'work', Number(root.dataset.hits || 8));
})();
